...
<li><strong>HubSpot form ID:</strong> submissions post to RLDatix HubSpot portal <code>27174408</code>, form GUID <code>7bbba4f2-2045-458d-a339-b06e5e7a16d7</code> (EU data centre), the dedicated Smart Match form. The ID is baked into the calculator file and is not something to edit in this package; if the form changes, RLDatix will supply an updated calculator file. Note that the calculator posts straight to the HubSpot Forms API, so it does not need, and will not respond to, the <code>hbspt.forms.create()</code> embed script.</li>
<li><strong>Do not put the calculator in a sandboxed iframe.</strong> A <code>sandbox</code> attribute without <code>allow-same-origin</code> blocks the HubSpot request and the PDF download while everything still looks normal on screen, so leads are lost with no visible error. If your CMS adds one, it needs at least <code>allow-scripts allow-same-origin allow-popups allow-downloads</code>. Point the iframe's <code>src</code> at the uploaded file rather than inlining the file into a <code>srcdoc</code> attribute.</li>
<li><strong>If HubSpot is unreachable</strong> (blocked network, CSP, ad-blocker): the visitor still gets their PDF, the lead is kept in the local browser backup below, nothing on your page breaks, and the reason is logged to the browser console.</li>
